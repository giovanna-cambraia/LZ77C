...
static int outbuf_push(outbuf_t *ob, uint8_t byte) {
    if (outbuf_ensure(ob, ob->len + 1) != 0) return -1;
    ob->buf[ob->len++] = byte;
    return 0;
}
...
int lz77_decompress(const uint8_t *input, size_t input_len, uint8_t **out_buf, size_t *out_len) {
    ...
    size_t target_len = ((size_t)len_hi << 32) | (size_t)len_lo;
    ...
    while (ob.len < target_len) {
        ...
        if (flag == 0) {
            uint32_t byte_val;
            if (br_read_bits(&br, 8, &byte_val) != 0) goto fail;
            if (outbuf_push(&ob, (uint8_t)byte_val) != 0) goto fail;
        } else {
            ...
            if (distance == 0 || distance > ob.len) {
                // corrupt stream: distance must point within already decored output.
                goto fail;
            }
            ...
            if (outbuf_ensure(&ob, ob.len + length) != 0) goto fail;
